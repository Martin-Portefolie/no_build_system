...
export default class extends Controller {
    ...
    updateUrl(weekNumber, year) {
        const url = `/test/weeks/${weekNumber}-${year}`; // Make sure to target the test route
        history.pushState({ path: url }, '', url);
    }

    // Fetch the data for the specific week and year using an AJAX request
    fetchWeekData(weekNumber, year) {
        fetch(`/test/weeks/${weekNumber}-${year}`)
            .then(response => response.text()) // Fetch the HTML content
            .then(html => {
                document.querySelector('#test-frame').innerHTML = html; // Update the content of the turbo frame
            })
